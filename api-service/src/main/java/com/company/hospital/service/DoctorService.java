...
public class DoctorService {
    ...
    public DoctorService(RestTemplate restTemplate, ApplicationProperties applicationProperties) {
        this.restTemplate = restTemplate;
        this.baseUri = URI.create(PROTOCOL + applicationProperties.getUrl());
    }

    public List<String> getTop10Doctors() {
        var uri = baseUri.resolve(DOCTOR_PATH + "/top");
        var exchange = restTemplate.exchange(uri, HttpMethod.GET, null, new ParameterizedTypeReference<List<String>>() {
        });
        if (exchange.getStatusCode().is2xxSuccessful()) {
            return exchange.getBody();
        } else {
            throw new RestClientException("Error getting top 10 doctors, status=%d".formatted(exchange.getStatusCode().value()));
        }
    }
}
